...
void main() {
  group('GdprService - RGPD Enhanced Features', () {
    ...
    group('RGPD Compliance Validation', () {
      test('should maintain valid consent check', () async {
      ...
      test('should validate consent is still valid within 1 year', () async {
        SharedPreferences.setMockInitialValues({'gdpr_consent_given': true, 'gdpr_consent_date': (DateTime.now().subtract(const Duration(days: 100))).toIso8601String()});
        await gdprService.checkConsentStatus();

        expect(gdprService.isConsentStillValid(), true);
      });

      test('should invalidate consent older than 1 year', () async {
        SharedPreferences.setMockInitialValues({'gdpr_consent_given': true, 'gdpr_consent_date': (DateTime.now().subtract(const Duration(days: 400))).toIso8601String()});
        await gdprService.checkConsentStatus();

        expect(gdprService.isConsentStillValid(), false);
      });

      test('should suggest consent renewal after 10 months', () async {
        SharedPreferences.setMockInitialValues({'gdpr_consent_given': true, 'gdpr_consent_date': (DateTime.now().subtract(const Duration(days: 305))).toIso8601String()});
        await gdprService.checkConsentStatus();
